...
- state `pending` or `cancelled` (do not persist raw promises/errors);
...
- on failure, retain a visible failed record for user retry; do not loop
  continuously;
...
- if it later succeeds, call authenticated `deleteItem`, which plan 003 makes
  release the operation ledger, then remove reviewed/pending state;
- if completion already happened, unmark reviewed, delete the item, and clear
  the operation only through `deleteItem`. If the `itemId` is unknown because the
  in-flight promise was lost across a lifecycle transition, resolve it with
  plan 003's read-only `getImportOperation` — never `beginImageImport`;
- catch and surface delete failure rather than discarding the recovery record.
